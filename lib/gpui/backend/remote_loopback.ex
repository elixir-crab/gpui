defmodule GPUI.Backend.RemoteLoopback do
  ...
  This backend speaks `GPUI.Protocol.Runtime` to a loopback server process. It is
  ...
  def open_window(%{server: server}, window_payload) do
    :ok = Server.request(server, RuntimeProtocol.open_window(window_payload))
  end

  @impl GPUI.Backend
  def update_window(%{server: server}, window_id, tree) do
    :ok = Server.request(server, RuntimeProtocol.update_window(window_id, tree))
  end

  @impl GPUI.Backend
  def drain_events(%{server: server}) do
    Server.request(server, RuntimeProtocol.drain_events())
  end

  @impl GPUI.Backend
  def emit_test_event(%{server: server}, event) do
    Server.request(server, RuntimeProtocol.event(normalize_test_event(event)))
  end
