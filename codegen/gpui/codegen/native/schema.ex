defmodule GPUI.Codegen.Native.Schema do
  ...
  @spec registry_source() :: String.t()
  def registry_source do
  ...
  defp generated_component_renderer(components) do
    arms =
      ...
      |> Enum.map(fn component ->
        ...
        render = String.to_atom("render_#{component.kind}")

        args =
          if component.kind == :input_component,
            do: [:element_id, :node, :context],
            else: [:node, :context]

        %AST.Arm{
          pattern: P.path_tuple([:ElementNode, variant], [:node]),
          body: [
            A.return_stmt(A.path_call([:element, :component, render], args))
          ]
        }
      end)
      |> Kernel.++([
