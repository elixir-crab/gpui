Code.require_file("repository_source.exs", __DIR__)

defmodule Examples.ElixirWorkbench.RepositoryTree do
  @moduledoc false
...
defmodule Examples.ElixirWorkbench.RepositoryModel do
  @moduledoc false
...
defmodule Examples.ElixirWorkbench.RepositoryView do
  use GPUI.View

  alias Examples.ElixirWorkbench.RepositoryModel, as: Model
  alias GPUI.UI
      ...
      <div class="flex flex-col gap-3 p-4" style={[background: {:rgb, 0x1E293B}]}>
        <div class="flex items-center justify-between gap-4">
          <div class="flex flex-col gap-1">
            <text class="text-white text-2xl font-semibold">Repository workspace</text>
...
defmodule Examples.ElixirWorkbench.RepositoryApp do
...
defmodule Examples.ElixirWorkbench.RepositoryCoordinator do
  use GenServer
  ...
  def handle_info({:scan_complete, scan_id, {:ok, repository}}, state) do
    if active_job?(state.scan_task, scan_id) do
      ...
      notify(state.owner, {:elixir_workbench_repository, :loaded, scan_id})
      {:noreply, state}
    else
      {:noreply, state}
    end
  ...
  def handle_info({:scan_complete, scan_id, {:error, message}}, state) do
    if active_job?(state.scan_task, scan_id) do
      ...
      notify(state.owner, {:elixir_workbench_repository, :scan_failed, scan_id, message})
  ...
  def handle_info({:preview_complete, job_id, {:ok, preview}}, state) do
    if active_job?(state.preview_task, job_id) do
      ...
      notify(state.owner, {:elixir_workbench_repository, :previewed, job_id})
  ...
  def handle_info({:preview_complete, job_id, {:error, message}}, state) do
    if active_job?(state.preview_task, job_id) do
      ...
      notify(state.owner, {:elixir_workbench_repository, :preview_failed, job_id, message})
  ...
  defp publish_tree(state, assigns) do
    ...
    notify(state.owner, {:elixir_workbench_repository, :tree_slice, assigns.tree_generation})
    state
  end
  ...
  defp publish_preview(state, assigns) do
    ...
    notify(
      state.owner,
      {:elixir_workbench_repository, :preview_slice, assigns.preview_generation}
    )
  ...
  defp maybe_report_task_exit(owner, kind, reason),
    do: notify(owner, {:elixir_workbench_repository, :task_exit, kind, reason})

  defp notify(nil, _message), do: :ok
  defp notify(owner, message), do: send(owner, message)
end

defmodule Examples.ElixirWorkbench.RepositorySupervisor do
  use Supervisor
  ...
  def init(opts) do
    task_supervisor =
      Keyword.get(opts, :task_supervisor, Examples.ElixirWorkbench.RepositoryTaskSupervisor)
    ...
    children = [
      ...
      {Examples.ElixirWorkbench.RepositoryCoordinator, coordinator_opts}
    ]
