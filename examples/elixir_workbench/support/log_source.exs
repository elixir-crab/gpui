...
defmodule Examples.ElixirWorkbench.LoggerHandler do
  @moduledoc false
...
defmodule Examples.ElixirWorkbench.LogView do
  use GPUI.View
      ...
      <div class="flex flex-col gap-3 p-4" style={[background: {:rgb, 0x1E293B}]}>
        <div class="flex items-center justify-between gap-4">
          <div class="flex flex-col gap-1">
            <text class="text-white text-2xl font-semibold">Runtime log stream</text>
    ...
    {:noreply,
     %{
       ...
       | events: [],
         ...
         dropped_count: 0,
         copy_count: 0,
         range: Examples.ElixirWorkbench.LogModel.initial_range(),
         generation: assigns.generation + 1,
         filter_status: :ready
     }}
  end
...
defmodule Examples.ElixirWorkbench.LogApp do
  use GPUI.Application

  alias Examples.ElixirWorkbench.LogModel, as: Model
...
defmodule Examples.ElixirWorkbench.LogSource do
  use GenServer

  alias Examples.ElixirWorkbench.LogEvent, as: Event
  alias Examples.ElixirWorkbench.LoggerHandler
  alias Examples.ElixirWorkbench.LogModel, as: Model
  alias GPUI.Runtime
    ...
    state =
      cond do
        "clear_events" in event_names ->
          notify(state.owner, {:elixir_workbench_log, :controls, event_names})
  ...
  def handle_info(
    ...
    notify(state.owner, {:elixir_workbench_log, :filter_failed, message})
  ...
  defp append_normalized(state, event, publish?) do
    ...
    notify(state.owner, {:elixir_workbench_log, :ingested, event.id})
  ...
  defp publish(state) do
    ...
    notify(
      state.owner,
      {:elixir_workbench_log, :published, assigns.generation, slice.total, length(slice.events),
       state.revision}
    )
  ...
  defp maybe_attach_logger(state, opts) do
    if Keyword.get(opts, :attach_logger, false) do
      handler_id = Keyword.get(opts, :handler_id, :gpui_elixir_workbench_log)
...
defmodule Examples.ElixirWorkbench.DemoProducer do
  use GenServer
...
defmodule Examples.ElixirWorkbench.LogSupervisor do
  use Supervisor
  ...
  def init(opts) do
    ...
    children = [
      ...
      {Examples.ElixirWorkbench.LogSource,
       name: Keyword.get(opts, :source_name, Examples.ElixirWorkbench.LogSource),
       runtime: runtime,
    ...
    children =
      if Keyword.get(opts, :demo, true) do
        children ++
          [
            {Examples.ElixirWorkbench.DemoProducer,
