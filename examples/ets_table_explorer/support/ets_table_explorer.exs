defmodule Examples.EtsTableExplorer.Model do
  @moduledoc false
...
defmodule Examples.EtsTableExplorer.Source do
  @moduledoc false

  use GenServer

  alias Examples.EtsTableExplorer.Model
...
defmodule Examples.EtsTableExplorer.View do
  use GPUI.View
      ...
      <div class="flex flex-col gap-3 p-4" style={[background: {:rgb, 0x1E293B}]}>
        <div class="flex items-center justify-between gap-4">
          <div class="flex flex-col gap-1">
            <text class="text-white text-2xl font-semibold">ETS table explorer</text>
  ...
  def handle_event("refresh_tables", _event, assigns) do
    if source = GenServer.whereis(assigns.source),
      do: Examples.EtsTableExplorer.Source.refresh(source)
  ...
  defp update_request(assigns, changes) do
    ...
    if source = GenServer.whereis(assigns.source) do
      Examples.EtsTableExplorer.Source.request(source, request(assigns))
    end
...
defmodule Examples.EtsTableExplorer.App do
  use GPUI.Application

  @impl GPUI.Application
  def mount(args) do
    args = Map.new(args)
    request = Examples.EtsTableExplorer.Model.initial_request()

    {:ok,
     [
       window "ETS Table Explorer" do
         size(1280, 760)

         root(
           Examples.EtsTableExplorer.View,
           Map.merge(request, %{
             source: Map.get(args, :source, Examples.EtsTableExplorer.Source),
             tables: [],
             table_total: 0,
             table_offset: 0,
...
defmodule Examples.EtsTableExplorer.Supervisor do
  @moduledoc false
  ...
  @impl Supervisor
  def init(opts) do
    source = Keyword.get(opts, :source, Examples.EtsTableExplorer.Source)
    task_supervisor = Module.concat(source, TaskSupervisor)

    children = [
      {Task.Supervisor, name: task_supervisor},
      {Examples.EtsTableExplorer.Source,
       runtime: Keyword.fetch!(opts, :runtime),
       name: source,
       task_supervisor: task_supervisor,
       interval: Keyword.get(opts, :interval, 2_000),
       table_source:
         Keyword.get(opts, :table_source, &Examples.EtsTableExplorer.Model.scan_tables/0)}
    ]
