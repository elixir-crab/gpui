...
pub(crate) fn apply_generated_input_events(
    ...
    initialize_generated_input_value(&runtime, &input_id, &value);
        ...
        element = element.on_click(move |_event, _window, _cx| {
            let _ = push_event(
                ...
                NativeEvent::Input {
                    ...
                    window_id,
                    event: event.clone(),
                    value: generated_input_value(&runtime_for_change, &input_id_for_change),
                },
            );
        });
    }
    ...
    if let Some(event) = keydown {
        ...
        element = element.on_key_down(move |key_event, _window, _cx| {
            ...
            if let Some(updated_value) =
                mutate_generated_input_value(&runtime_for_change, &input_id_for_keydown, &key_event.keystroke)
            {
...
#[cfg(feature = "real-gpui")]
pub(crate) fn initialize_generated_input_value(
    runtime: &ResourceArc<RuntimeResource>,
    input_id: &str,
    value: &str,
) {
...
#[cfg(feature = "real-gpui")]
pub(crate) fn generated_input_value(
    runtime: &ResourceArc<RuntimeResource>,
    input_id: &str,
) -> Option<String> {
    runtime
