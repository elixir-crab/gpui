...
defmodule GPUI.ElixirWorkbenchLogSourceTest do
  ...
  alias Examples.ElixirWorkbench.LogSource, as: Source
  ...
  test "source retains bounded history while pause, follow, filtering, and clear stay controlled" do
    ...
    assert_receive {:elixir_workbench_log, :controls, events}
  ...
  test "OTP Logger handler forwards bounded events without owning source state" do
    ...
    assert_receive {:elixir_workbench_log, :ingested, "event-1"}
    await_published(1, 1)
  ...
  defp await_published(total, minimum_revision) do
    receive do
      {:elixir_workbench_log, :published, _generation, ^total, _loaded, revision}
