...
defmodule GPUI.ElixirWorkbenchRepositorySourceTest.FakeRepository do
...
defmodule GPUI.ElixirWorkbenchRepositorySourceTest do
  ...
  test "supervises scans and replaces an active preview without stale results" do
    ...
    assert_receive {:elixir_workbench_repository, :loaded, 1}
    ...
    assert_receive {:elixir_workbench_repository, :previewed, 2}
    ...
    assert_receive {:elixir_workbench_repository, :loaded, 2}
    assert_receive {:elixir_workbench_repository, :previewed, 3}
    ...
    assert_receive {:elixir_workbench_repository, :preview_slice, ^preview_generation}
    ...
    change(runtime, "status_filter_changed", "untracked")
    assert_receive {:elixir_workbench_repository, :tree_slice, ^tree_generation}
  ...
  defp repository_fixture! do
    root =
      Path.join(
        System.tmp_dir!(),
        "gpui-workbench-repository-#{System.unique_integer([:positive])}"
      )
