...
defmodule GPUI.EtsTableExplorerExampleTest do
  use GPUI.Test, async: false

  alias Examples.EtsTableExplorer.App
  alias Examples.EtsTableExplorer.Model
  alias Examples.EtsTableExplorer.Source

  @source Examples.EtsTableExplorer.TestSource
  @task_supervisor Examples.EtsTableExplorer.TestTaskSupervisor
  ...
  test "supervised source inspects a live ETS table asynchronously" do
    table = :ets.new(:gpui_ets_explorer_fixture, [:set, :public])
    ...
    metadata = [
      %{
        id: inspect(table),
        tid: table,
        name: ":gpui_ets_explorer_fixture",
        owner: inspect(self()),
        type: "set",
        protection: "public",
        size: 2,
  ...
  defp await_assigns(runtime, predicate) do
    ...
    if predicate.(current) do
      ...
      after
        5_000 -> flunk("ETS explorer did not publish the expected snapshot")
      end
    end
  end
end
